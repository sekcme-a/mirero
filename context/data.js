...
export function DataProvider(props){
  ...
  const fetch_thumbnails_list = async (type) => {
    const query = await db.collection("team").doc("miraero").collection("posts").where("condition", "==", "게제중").where("type","==",type).orderBy("publishedAt", "desc").get()
  ...
  const fetch_post = async (postId) => {
    const doc = await db.collection("team").doc("miraero").collection("posts").doc(postId).get()
    if (doc.exists) {
