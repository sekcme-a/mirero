      ...
      <Grid container>
        <Grid item xs={12} md={9}>
          <div className={styles.written_info}>
            <p><strong>게제일:</strong> {formatDateToYYYYMMDD(data?.publishedAt.toDate())} / <strong>작성자:</strong> {data?.author}</p>
          </div>
          <h2>{data?.title}</h2>
          <div className={styles.content_container}>
            <div className="quill_custom_editor" style={{marginTop:"3px"}}>
              <div dangerouslySetInnerHTML={createMarkup()} className={styles.html_container} />
            </div>
          </div>
        ...
        {monitorSize<900 &&
            <div className={styles.navbar_container}>
              {/* <h6>{`< 이전글`}</h6> */}
              <h6 onClick={()=>{
                if(type==="tomorlove") router.back()
                else router.push(`/article/${type}`)
              }}>목록으로</h6>
              {/* <h6>{`다음글 >`}</h6> */}
            </div>
          }
