...
const IdAndPassword = ({style={}}) => {
  ...
  return(
    <div className={styles.main_container} style={style}>
      ...
      <Button fullWidth variant="contained" onClick={onLoginClick} color="success" sx={{mt:"20px"}}><div style={{fontWeight:"bold"}}>로그인</div> </Button>
      <Button fullWidth variant="contained" sx={{mt:"15px"}} color="warning" onClick={()=>router.push("/auth/signIn")}>회원가입</Button>
      <Button variant="text"  sx={{mt:'10px'}} onClick={()=>router.push("/auth/resetPassword")}><div style={{color:"#333"}}>비밀번호를 잊으셨나요?</div></Button>
    </div>
  )
}
